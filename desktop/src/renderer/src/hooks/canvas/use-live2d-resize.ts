...
const MIN_SCALE = 0.1;
const MAX_SCALE = 5.0;
const PET_INITIAL_SCALE_MAX = 0.85;
const PET_MAX_SCALE = 1.2;
...
const clampScale = (scale: number, isPet: boolean, initial = false) => {
  const maxScale = isPet
    ? (initial ? PET_INITIAL_SCALE_MAX : PET_MAX_SCALE)
    : MAX_SCALE;
...
export const useLive2DResize = ({
  ...
  const initialScale = clampScale(modelInfo?.kScale || DEFAULT_SCALE, isPet, true);
   ...
   */
  useEffect(() => {
    const newInitialScale = clampScale(modelInfo?.kScale || DEFAULT_SCALE, isPet, true);
  ...
  }, [isPet, modelInfo?.url, modelInfo?.kScale]);
  ...
  const animateEase = useCallback(() => {
    const clampedTargetScale = clampScale(targetScaleRef.current, isPet);
  ...
  const handleWheel = useCallback((e: WheelEvent) => {
    ...
    const newTargetScale = clampScale(currentActualScale + increment, isPet);
  ...
  }, [isPet, modelInfo?.scrollToResize, animateEase]);
  ...
  const handleResize = useCallback(() => {
    ...
    try {
      ...
      const shouldResize = shouldResizeLive2DCanvas({
        width,
        height,
        previousWidth: lastDimensions.width,
        previousHeight: lastDimensions.height,
        sidebarChanged,
        hasAppliedInitialScale: hasAppliedInitialScale.current,
      });

      if (!shouldResize) {
        if (width === 0 || height === 0) {
          console.warn('[Resize] Width or Height is zero, waiting for a valid container size.');
        }
