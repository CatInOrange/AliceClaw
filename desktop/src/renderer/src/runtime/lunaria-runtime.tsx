...
import { createTempFileComposerAttachment } from "@/runtime/composer-attachment-utils.mjs";
import { shouldRunAutomationRule } from "@/runtime/automation-utils.mjs";
import { resolveAssistantDisplayName } from "@/runtime/assistant-display-utils.mjs";
import { getConnectionStateAfterChatError } from "@/runtime/chat-runtime-utils.mjs";
import { resolveFocusCenterConfig } from "@/runtime/focus-center-utils.mjs";
import { resolveProviderFieldState } from "@/runtime/provider-field-state.mjs";
import { getProviderOverridesPayload } from "@/runtime/provider-overrides.mjs";
import {
  shouldUseGlobalCursorTracking,
  toRendererPointerFromScreenPoint,
} from "@/runtime/global-cursor-utils.mjs";
import {
  createRealtimeLipSyncCleanup,
  getActiveLive2DModel,
  getLipSyncPlaybackMode,
} from "@/runtime/live2d-audio-utils.mjs";
import {
  createNextPlaybackVersion,
  isPlaybackVersionCurrent,
  shouldSpeakRealtimeMessage,
} from "@/runtime/speech-runtime-utils.mjs";
import {
  applyFocusCenter,
...
interface LunariaRuntimeContextValue {
...
const LunariaRuntimeContext = createContext<LunariaRuntimeContextValue | null>(null);
...
export function LunariaRuntimeProvider({
  children,
}: {
  ...
  const resolvePendingCaptureAttachment = useCallback((attachmentId: string, payload: string | {
    ...
    const nextAttachment = typeof payload === "string"
  ...
  const value = useMemo<LunariaRuntimeContextValue>(() => ({
    reconnect,
    createNewSession,
    loadSession,
    sendComposerMessage,
  ...
  return (
    <LunariaRuntimeContext.Provider value={value}>
      {children}
    </LunariaRuntimeContext.Provider>
  );
}

export function useLunariaRuntime(): LunariaRuntimeContextValue {
  const context = useContext(LunariaRuntimeContext);
  if (!context) {
    throw new Error("useLunariaRuntime must be used inside LunariaRuntimeProvider");
  }
  return context;
}
